...
def generate_fold_data(data_path, fold_size, time_freq, year, fold):
    ...
    fold_start_week = weeks_list[fold - 1]
    fold_end_week = weeks_list[fold - 1] + fold_size - 1

    df = df[df['datetime'].dt.isocalendar().week.between(fold_start_week, fold_end_week)]
...
def save_stats(
    data_path, strategy_file_path, n_fold, year, logger, fold_size, time_freq,
    ...
    df_data = generate_fold_data(
        data_path=data_path, fold_size=fold_size, time_freq=time_freq, 
        year=year, fold=n_fold
    )
...
def calculate_port_in_sample_perf(data_path, port_file_path, logger, 
                                   n_fold, year, fold_size, time_freq,
