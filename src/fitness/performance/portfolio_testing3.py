...
def generate_fold_data_valid(data_path, fold_size, time_freq, fold, year):

    df = pd.read_csv(data_path)
    df['datetime'] = pd.to_datetime(df['datetime'])
    df = df[df['datetime'].dt.year==year]

    weeks_list = list(df['datetime'].dt.isocalendar().week.unique())
    weeks_list = sorted(weeks_list)

    valid_fold_week = weeks_list[fold - 1] + fold_size

    df = df[df['datetime'].dt.isocalendar().week==valid_fold_week]
...
def generate_fold_data_test(data_path, fold_size, time_freq, fold, year):
    ...
    valid_fold_week = weeks_list[fold - 1] + fold_size + 1
...
def test_out_of_fold(df_str, data_path, n_fold, fold_size, time_freq, year, create_txt_code=create_txt_code1):
...
def filter_save_lstr(data_path, n_fold, str_file_path, logger, fold_size, time_freq, year,
    ...
    if df_selected_str.shape[0] != 0:
        ...
        if is_subset:
            ...
            final_perf_df_fold, final_mc_df_fold, equity_curve_dict_fold) = test_out_of_fold(
                ...
                fold_size, time_freq, year, create_txt_code=create_txt_code
            )
        else:
            (final_entry_win_pc_df_fold, final_exit_win_pc_df_fold, final_core_win_pc_df_fold, 
            final_perf_df_fold, final_mc_df_fold, equity_curve_dict_fold) = test_out_of_fold(
                df_selected_str, data_path, n_fold, fold_size, time_freq, year, create_txt_code=create_txt_code
            )
...
def calculate_port_out_sample_perf(data_path, port_file_path, logger, 
                                   n_fold, fold_size, time_freq, year,
    ...
    df_prices = generate_fold_data_valid(
        data_path=data_path, fold_size=fold_size, time_freq=time_freq, fold=n_fold, year=year
    )
...
def calculate_str_out_sample_perf(data_path, port_file_path, logger,
    ...
    df_prices = generate_fold_data_valid(
        data_path=data_path, fold=n_fold, fold_size=fold_size, time_freq=time_freq, year=year
    )
